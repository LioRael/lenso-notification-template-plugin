...
pub struct NotificationTemplateOperator;

impl NotificationTemplateOperator {
    pub async fn setup(
        database_url: &str,
        schema: &str,
    ) -> Result<SetupOutcome, NotificationTemplateOperatorError> {
    ...
    pub async fn upgrade(
        database_url: &str,
        schema: &str,
    ) -> Result<UpgradeOutcome, NotificationTemplateOperatorError> {
        Ok(SchemaOperator::connect(database_url, schema_plan(schema)?)
            .await?
            .upgrade()
            .await?)
...
#[derive(Debug, Error)]
pub enum NotificationTemplateOperatorError {
    #[error(transparent)]
